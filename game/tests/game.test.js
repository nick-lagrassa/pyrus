...
test("ValidParenthesesGame runs correctly", () => {
  ...
  expect(game._board.players[1].hand).toHaveLength(
    settings.NUM_CARDS_DRAWN_AT_GAME_START
  );
  ...
  for (let i = 0; i < settings.NUM_PLAYER_MOVES_PER_TURN; i++) {
    ...
    if (i < settings.NUM_PLAYER_MOVES_PER_TURN - 1) {
      ...
      expect(game._board.players[0].hand).toHaveLength(
        settings.NUM_CARDS_DRAWN_AT_GAME_START +
          settings.NUM_CARDS_DRAWN_PER_TURN
      );
      expect(game._board.deck.cards.length).toBe(
        oldDeckSize - 1 - settings.NUM_CARDS_DRAWN_PER_TURN
      );
    }
  }
