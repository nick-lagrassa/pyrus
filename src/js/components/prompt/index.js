...
export default class Prompt extends Component {
  render() {
    ...
    const numTestsPassed = prompt._testResults
      ...
      : null;
    const shouldShowTests =
      !isWaitingForTestResults && prompt._testResults.length > 0;
        ...
        <div>
          ...
          {shouldShowTests && (
            <div className="pv3 br2 bg-pear-near-white">
              <p className="b">
                {numTestsPassed === prompt._testResults.length ? "✅" : "⚠️"}{" "}
                {numTestsPassed} out of {prompt._testResults.length} tests
                passed!
              </p>
              {prompt._testResults
                .filter(result => !result.passed)
                .map((result, i) => {
                  const shouldDisplayConsole =
                  ...
                  return (
                    <div className="bg-pear-yellow mv2 pa3 br2" key={i}>
                      ...
                      {shouldDisplayConsole && [
                        <p key="console-label" className="code lh-copy mv0">
                          Console:
                        </p>,
                        <div
                          key="console-logs"
                        ...
                        >
                          {result.console.map((log, i) => (
                            <pre className="mv0 lh-copy" key={i}>
                              {log}
                            </pre>
                          ))}
                        </div>
